...
A minimal RPC client/server middleware with TCP transport and `struct` serialization.
...
> Edn 5. Pearson Education 2012. Section 5.3.2, Implementation of RPC:
>
...
- `picorpc.py` -- pico-rpc runtime: IDL primitives and the Dispatcher.
- `rpcgen.py` -- interface compiler: idl -> client/server stubs.
- `math.idl.py` -- interface definition of the Math service.
- `server.py`, `client.py` -- programmer's code, using the generated stubs.
